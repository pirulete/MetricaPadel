...
> "Si no se mide, no se puede mejorar". El orquestador de cada workflow DEBE medir la duración real del loop con `--start`/`--finish` y registrar las métricas al cerrarlo (detalle completo en `ARCHITECTURE.md > Loop Engineering`).

```bash
# Al abrir el workflow (mide la duración real del loop)
node scripts/loop-metrics.js --start <change_id> [--module <mod>]

# Al cerrar el workflow (computa loopDurationMs real y registra el evento)
node scripts/loop-metrics.js --finish <change_id> --iterations <n> [--gates-failed <n>] [--pattern-matches <n>] [--escalated] [--module <mod>]
```
...
- Alternativa sin timer: `node scripts/loop-metrics.js --record <change_id> ...` (loopDurationMs explícito).
